...
Let $M$ be a flat compact $n$-manifold with a piecewise smooth boundary (e.g. $M = \mathbb{T}^{n_1} \times [0,1]^{n_2}$, where $[0,1]$ is the unit interval).
Let $X$ be a $C^2$ vector-field on $M$, tangential to the boundary. 
...
Similarly, (and in the opposite direction) for any trace class operator $A$ there is a unique $\rho_{A} \in L^1(M)$ such that: 
\begin{align*}
...
The map, ``$A \mapsto \rho_{A}$'', is merely the adjoint of ``$f \mapsto H_{f}$''. Therefore ``$A \mapsto \rho_{A}$'' is surjective.

Finally, for any vector-field $X$ which is tangential to the boundary of $M$ let us define the differential operator
\begin{align}
	\pounds_X [\psi] = \frac{1}{2} \left( \sum_{i=1}^{n} X^i \partial_i \psi + \partial_i (\psi X^i) \right) \label{eq:representation}
\end{align}
for any $\psi \in H^1(M) \subset L^2(M)$.
In fact, $\pounds_X$, is an anti-symmetric with respect to the  $L^2$-inner product.
By Stone's theorem, $\pounds_X$ is the infinitesimal generator of a one-parameter semi-group on the topological Lie group $\operatorname{O}(L^2(M))$.
We can relate this transformation to the flow of the vector field $\Phi_X^t:M \to M$ in a precise sense.
In particular
\begin{align*}
	( e^{\pounds_X} \cdot \psi )(x) =  \det( D [(\Phi_X^t)^{-1}](x) )\psi( ( \Phi_X^t)^{-1} (x) ).
\end{align*}
