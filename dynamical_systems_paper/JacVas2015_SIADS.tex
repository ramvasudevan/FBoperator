...
Given an initial density $\rho_{0} \in \Dens(S^{1})$ the exact solution at time $t$ is given by
\begin{align}
	\rho(x;t) = \dots \rho_{0}( \dots ) \label{eq:exact solution}
\end{align}
\todo[inline]{figure this out}  In figure \ref{fig:S1} three subfigures depict $\rho(x;t)$ at $t=2$ with an initial condition given by a uniform distribution.
...
Moreover, the $L^{1}$-norm is not conserved in standard spectral discretization.  However, it is conserved in GN discretization.
