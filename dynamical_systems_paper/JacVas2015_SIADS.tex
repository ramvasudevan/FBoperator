...
\todo[inline]{We must still show that $E(0) \to 0$, spectrally, and that the convergence proofs for functions and densities follows}

\begin{thm} \label{thm:convergence 1}
	Let $\psi_{0} \in H^{s}(M)$ for $s > 1$ and let $\psi(t)$ be the exact and $\tilde{\psi}(t)$ be the numerical solutions on the interval $[0,T]$ for $T < \infty$.
	Then $\| \psi(t) - \tilde{\psi}(t) \|_{H^{s}} \leq ( K_{T} n^{-2(s-1)} t+  E(0) ) e^{C_{T}}$ for $t \in [0,T]$ and constants $K_{T}$ and $C_{T}$.
\end{thm}
...
\begin{proof}
	Let $\Delta \psi  = \psi - \tilde{\psi}$, $E = \| \Delta \psi \|_{H^{s}}$ for $s > 1$.  Note that $\frac{dE}{dt} = \frac{1}{2E} \langle \Delta \psi \mid \frac{d}{dt} \Delta \psi \rangle_{H^{s}}$.
	By the Cauchy-Schwarz inequality we see that
	\begin{align*}
		\frac{dE}{dt} \leq \left| \frac{dE}{dt} \right| &\leq \| X[\psi] - \pi_{n}(X[\tilde{\psi}]) \|_{H^{s}} = \| X[\psi] - \pi_{n}( X[\psi- \Delta \psi]) \|_{H^{s}} \\
		&\leq \| (1-\pi_{n})|_{H^{s-1}} \| \, \|X|_{H^{s}} \| \, \| \psi \|_{H^{s}} + \| \pi_{n} \| \, \|X|_{H^{s}} \| \, \| \Delta \psi \|_{H^{s}} \\
	\end{align*}
	...
	\begin{align*}
		\frac{dE}{dt} \leq C_{1,T} \, \| (1- \pi_{n})|_{H^{s-1}} \| + C_{2,T} E
	\end{align*}
	However the operator norm of $(1-\pi_{n})|_{H^{s-1}}$ decays like $\lambda_{n}^{-(s-1)}  \sim n^{-2(s-1)}$.
	Therefore we may write this inequality in a form suitable to invoke Lemma \ref{lem:Gronwall} with $\epsilon \sim n^{-2(s-1)}$.
	As a result we find
	\begin{align*}
		E(t) \leq ( K_{T} n^{-2(s-1)} t+  E(0) ) e^{C_{2,T}}
	\end{align*}
\end{proof}
