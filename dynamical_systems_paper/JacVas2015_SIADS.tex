...
\begin{itemize}
	...
	\item The adjoint of an operator $L : \mathcal{H} \to \mathcal{H}$ will be denoted by $L^{\dagger}$.
	\item The trace of a trace class operator, $L$, will be denoted by $\Tr(L)$.
	\item The commutator bracket for operators $A,B$ on $\mathcal{H}$ will be denoted by $[A,B] := A \cdot B - B \cdot A$.
\end{itemize}
...
\begin{theorem} \label{thm:sparsity}
	Let $x^{1},\dots,x^{n}$ be a dense coordinate chart for $M$\footnote{Such a chart always exists on a compact manifold by choosing a Riemannian metric and extending a Riemannian exponential chart to the cut-locus.\cite{Sakai1996,MO_dense_charts} },
	...
	If the vector-field $X$ is such that $X^{i} = \sum_{k} c_{k}^{i} f_{k}$ with fewer than $K>0$ of $c^{i}_{k}$'s being non-zero for each $i=1,\dots,n$, then the operator $X_{m}$ in Algorithm \ref{alg:half density} is banded diagonal with fewer than $nK$ bands.
\end{theorem}

\begin{corollary} \label{cor:sparsity}
	If the same hypotheses of Theorem \ref{thm:sparsity} are satisfied, then the integration step of Algorithm \ref{alg:half density} is of complexity $\mathcal{O}( K m n)$.
\end{corollary}

Theorem \ref{thm:sparsity} and Corollary \ref{cor:sparsity} suggest selecting a basis where $K$ is small.
...
This yields Algorithm \ref{alg:density} to spectrally discretize \eqref{eq:density pde} in a qualitatively accurate manner for densities.
