...
We will see that this approximation converges exponentially when $\psi$ is smooth, and converges at a rate $n^{1-s}$ when $\psi \in H^{s}$.

This gives us the following numerical scheme for \eqref{eq:half density pde} for a fixed $n$:

\begin{enumerate}
	\item Given the initial condition $\psi_0 \in L^2(S^1)$ compute ${\bf c}_n$.
	\item Solve the ode $\frac{d {\bf c}_n }{dt} = {\bf X}_n \cdot {\bf c}_n$ on $\mathbb{C}^{2n+1}$ over a finite time interval, $[0,T]$, using a legacy ode integration scheme or trusted software (e.g. we use vode).\footnote{If ${\bf X}_n$ is constant in time, than one can solve this using a numerical matrix exponential as well, but this can be treacherous.}
	\item Let $\psi^{(n)}(x,t) = \sum_{k=-n}^n c_k(t) e^{2\pi ikx}$. 
\end{enumerate}
We will find that $\psi^{(n)}$ is a good approximation to the exact solution of \eqref{eq:half density pde} when $\psi_0$ is sufficiently regular.

Through \ref{thm:quantize} we will find that $\rho^{(n)} = ( \psi^{(n)} )^2$ is a provides a good approximation to \eqref{eq:density pde}.
...
\begin{figure}
	...
	\caption{ \tiny {\bf Top:} GN Spectral, {\bf Middle:} Monte Carlo, {\bf Bottom:} Standard Spectral}
	\label{fig:2 torus}
\end{figure}
