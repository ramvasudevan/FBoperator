...
The exact solution, $\hat{f}$, is represented via the spectral theorem by looking at it's level sets (see Appendix \ref{app:functions}).
...
Finally the result of using a standard spectral scheme (\ref{alg:spectral function}) is depicted in figure \ref{fig:standard function 1}.
...
In particular, given the high cost of computation for Algorithm \ref{alg:function}  vs Algorithm \ref{alg:spectral function} ($O(n^{2})$ vs $O(n)$) we generally do not recommend this algorithm in practice.
However, Algorithm \ref{alg:function} is of theoretical value, in that it may inspire new ways of discretization (as discussed in )\todo{Include your level set idea somewhere}
...
\begin{align*}
	g(x,t) =  \frac{ \cos(x) }{ \sqrt{ e^{4t} \sin^{2}(x) + \cos^{2}(x) } }.
\end{align*}
If we then consider the initial condition $h_{0} = f_{0} \cdot g_{0}  = \sin(x) \cos(x)$
then the exact solution is
\begin{align*}
	h(x,t) = f(x,t) g(x,t) = \frac{ \cos(x) \sin(x) }{ \cos^{2}(x) + e^{4t} \sin^{2}(x) }.
\end{align*}
...
If one uses algorithm \ref{alg:function} than both of these options yield the same result up to time discretization error (which is obtained with error tolerance $1e-8$ in our code).
If one uses algorithm \ref{alg:spectral function} than these options yield different results with a discrepancy.
We plot this discrepancy in \ref{fig:discrepancy}.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{./images/function_plots/discrepancy}
	\caption{The discrepancy due to non-preservation of scalar products by Algorithm \ref{alg:spectral function}}
...
We can see that the operator-norm behaves well using Algorithm \ref{alg:function}.  This should be clear without plotting, as Algorithm \ref{alg:function} evolves an operator by
an iso-spectral flow, and therefore must conserve the operator norm.
We see that the operator-norm is conserved to machine precision for Algorithm \ref{alg:function}, while it will drift using standard discretization.
In contrast, the sup-norm of the output from a standard spectral discretization (Algorithm \ref{alg:spectral function}) is not conserved.
We observe the operator norm drifting upwards.  This is all depicted in figure \ref{fig:norms}
