...
\begin{abstract}
	...
	Particle based methods conserve these properties, but they do not converge as quickly as spectral methods on smooth data.
...
then so is the product, $f \cdot g$, and the sum, $f+g$.
...
However, the standard spectral algorithm is not qualitatively accurate under Definition \ref{def:quality}.
...
Various galerkin discretization of the Koopman operator\footnote{Briefly, the Koopman operator a linear operator ``$K$'' which yields the solution $f = K \cdot f_{0}$ to \eqref{eq:function pde}. We refer the reader to \cite{BudisicMohrMezic2012} for a survey of recent applications.} have been successfully used
in a variety of contexts for generic dynamic systems \cite{BudisicMohrMezic2012,Mezic2005}, most notably fluid systems \cite{Rowley2009} where such discretization serve as a generalization of dynamic mode decomposition \cite{Schmid2010}.
Dually, Ulam-type discretizations of the Frobenius-Perron operator  \cite{Ulam1947,LasotaMackey1994} can be used to find invariant manifolds of systems with uniform Gaussian noise \cite{FroylandJungeKoltai2013,FroylandPadberg2009}.
In continuous time, Petrov-Galerkin discretization of the infinitesimal generator of the Frobenius Perron operator converge in the presence of noise \cite{BittracherKoltaiJunge2015}
and preserved positivity in a Haar basis \cite{koltai2011thesis}.
...
It is difficult to determine where this action first appeared, but it was known to representation theorists \cite{VershilGelfandGraev1975,Ismagilov1975}
and people involved with quantum probability theory \cite{Meyer1998,Parthasarathy2012}.
To be more specific, a half-density can be abstractly summarized as an objects whos ``square'' is a density.
Alternatively, it can be understood as the mathematician's nomenclature for the physicists ``wave functions''.
One of the benefits of working with half-densities, over probability densities, is that the space of half-densities is a Hilbert space, while the space of probability densities is a convex cone.
This ``square-root'' type of transformation to a Hilbert space has been used to solve a nonlinear problem throughout mathematics, such as the invention of the complex numbers to solve polynomials.
...
This is in contrast to the standard $L^{2}$-space, in which a volume form, or a reference density must be chosen first \cite{Adams2003}.
The cost we pay is that the elements of $L^{2}(M)$ are not functions, but something less familiar, they are half-densities.
...
\end{align}
By the Stone's theorem, there is a solution for all time, in spite of the fact that the Lie derivative is unbounded.
...
In fact, $U(t)$ is merely the push-forward operator given in \eqref{eq:transformation law} with ``$\Phi$'' replaced by the flow-map of the vector-field, $X$.

\subsection{The relationship with classical $L^{2}$ spaces}
\label{sec:classical_Lebesgue}
For any manifold $M$ (possibly non-orientable) one can assert the existence of a smooth non-negative density $\mu$ \cite[Chapter 16]{Lee2006}.
...
For each $X \in \mathfrak{X}(M)$ the operator $\pounds_{X}$ on $L^{2}(M)$ of \eqref{eq:representation} is unbounded.
The domain of $\pounds_{X}$ is
\begin{align}
	\operatorname{dom}( \pounds_{X} ) := \{ \psi \in L^{2}(M) \mid  \| \pounds_{X}[\psi] \|_{L^{2}} < \infty \}.
\end{align}
One can show that $\operatorname{dom}( \pounds_{X} )$ is a closed subspace of $L^{2}(M)$ and define
...
%This fractional power of $\pounds_{X}$ allows us to consider Sobolev spaces of fractional orders as well.
  ...
  However, while the canonicalism of $L^{2}(M)$ will be useful,
  \emph{canonical} Sobolev spaces will not be.
  We will find our algorithms converge on Sobelev spaces and we will need to choose a norm.
  The choice of a norm is not canonical, and so we may just as well use the standard metric dependent defintions.
  
To begin, equip $M$ with a Riemannian metric $g:TM \oplus TM \to \mathbb{R}$.
The metric, $g$, induces a positive density $\mu_g$,
known as the \emph{metric density} and an inner-product on $C^\infty(M)$
given by
\begin{align}
...
and the Helmholtz operator, $1 + \Delta$, is  a positive definite operator
...
In this section we will spectrally discretize \eqref{eq:function pde} and \eqref{eq:density pde}.
In order to realize this goal we must obtain canonical relationships between functions, densities, and vector-fields with $L^{2}(M)$ which are
amenable to spectral discretizion.
...
We see that the map, $f \mapsto \hat{f}$, preserves the ring structure because $\widehat{f \cdot g + h} = \hat{f} \cdot \hat{g} + \hat{h}$.

Similarly, (and in the opposite direction) for any trace class operator $\hat{\rho}:L^{2}(M) \to L^{2}(M)$ there is a unique distribution $\rho \in C(M)'$ such that 
\begin{align}
...
The benefit of using \eqref{eq:quantum observable ode} and \eqref{eq:quantum density ode} to represent the PDEs of concern
is that we may discretize \eqref{eq:quantum observable ode} and \eqref{eq:quantum density ode} using a standard least squares projections on $L^{2}(M)$.
...
Assumption \ref{ass:basis} holds for a variety of choices, such as the Fourier basis itself, as well as any Sturm-Louiville basis (e.g. Legendre functions).
...
This article is concerned purely with semi-discretization of \eqref{eq:function pde} and \eqref{eq:density pde}, and so we will assume that this ode solver 
...
It is not possible to take the square root of an arbitrarily irregular density, such as a Dirac delta functional.
...
We found that conservation laws completely characterize \eqref{eq:function pde} and \eqref{eq:density pde}, and this explains the benefits of using qualitatively accuracte scheme at a more fundamental level.
