...
and so the solutions are of the form $H_{f,N}(t) = U_{N}(t) \cdot H_{f,N}(0) \cdot U_{N}(t)^{\dagger}$ for all time $t$ for some unitary transformation $U_{N}(t)$.
In fact, $U_{N}(t)$ is the same unitary transformation as the one in Proposition \ref{prop:unitary}.
...
\begin{theorem} \label{thm:norms}
	The norms $\| \psi_{N} (t)\|_{L^{2}}, \|\rho_{N}(t)\|_{L^{1}},$ and $\|\hat{f}_{N}(t) \|_{op}$ are constant with respect to $t$ and
	\begin{align}
...
\begin{theorem} \label{thm:algebra}
	...
	\end{align}
	Moreover, $H_{k,N}(t)$ strongly converges to $H_{k}$ as $N \to \infty$ in the operator norm on $H^{s}(M)$ when $f,g,h \in W^{s,\infty}(M)$ for $s>1$.
\end{theorem}
\begin{proof}
