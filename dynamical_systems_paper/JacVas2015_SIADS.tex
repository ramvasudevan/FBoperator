...
The metric also induces an elliptic operator, known as the Laplace-Beltrami operator $\Delta: C^{\infty}(M) \to C^{\infty}(M)$, which is negative-semidefinite (i.e. $\int f \, \Delta f \, \mu_{g} \leq 0$ for all $f \in C^{2}(M)$) \ram{Is it worth just defining this equation in this paper?} \hoj{Nah.  JNum readers ought to know this.}.
...
We will see that $\psi_{N}(t)$ evolves by unitary transformations, just as the exact solution to \eqref{eq:half density pde} does.
This correspondence will be key in providing the qualitative accuracy of algorithms that follow, so we formally state it here.
...
\begin{theorem} \label{thm:half density convergence}
	...
	\begin{align}
		\varepsilon_{N}(t) \leq \| \psi(0) \|_{H^{\bar{s}}} \, K_{T} \left( N^{-2(s-1)} t+  \frac{n}{\bar{s}-s} N^{-2(\bar{s}-s)/n} \right) e^{C_{T} t}
	\end{align}
	where $K_{T}$ and $C_{T}$ are positive and constant with respect to $N$,$s$, and $\bar{s}$.
	In particular for $s = (\bar{s}+1)/2$:
	\begin{align}
			\varepsilon_{N}(t) \leq \| \psi(0) \|_{H^{\bar{s}}} \, K_{T} \left( N^{1-\bar{s}} t+  \frac{n}{\bar{s}-1} N^{(1-\bar{s})/n} \right) e^{C_{T} t}.
	\end{align}
\end{theorem}
...
\begin{proof}[Proof of Theorem \ref{thm:half density convergence}]
	...
	\begin{align}
		\frac{d\varepsilon_{N}}{dt} \leq K' \, \| (1- \pi_{N})|_{H^{s-1}} \| \, \| \psi_{0}\|_{H^{s}}+ C_{T} \varepsilon_{N}
	\end{align}
	for constants $C_{T}$ and $K'$.
	As $s > 1$, for sufficiently large $N$ we can compute that $\| (1-\pi_{N})|_{H^{s-1}} \|_{op} \leq (1+\lambda_{N+1})^{-(s-1)}$ where $\lambda_{N}$ denotes the $N$th eigenvalue of the Laplace operator.
	...
	Therefore by Lemma \ref{lem:Gronwall} with $\epsilon = C_{T} n^{-2(s-1) / d} \, \| \psi_{0}\|_{H^{s}}$:
	\begin{align}
		\varepsilon_{N}(t) \leq ( K' N^{-2(s-1) / n} \| \psi_{0} \|_{H^{s}} t+  \varepsilon_{N}(0) ) e^{C_{T} t}.
	\end{align}
...
\begin{theorem} \label{thm:density convergence}
	...
	Then:
	\begin{align}
		\| \rho(t) - \rho_{N}(t) \|_{L^{1}} \leq \| \rho(0) \|_{W^{\bar{s},1}} \, K \left( N^{-2(s-1)} t+  \frac{d}{\bar{s}-s} N^{-2(\bar{s}-s)/n} \right) e^{C_{T} t}
	\end{align}
...
\begin{proof}
	...
	for $s>0$, then:
	\begin{align*}
		\| H_{f} - H_{f,N} \|_{H^{s},op} &= \sup_{\| \psi \|_{H^{s}}=1} \langle \psi \mid H_{f} - H_{f,N} \mid \psi \rangle \\
			&= \sup_{\| \psi \|_{H^{s}}=1} \left( \langle \psi \mid H_{f}  \mid \psi \rangle - \langle \psi - \pi_{N}^{\perp}(\psi) \mid H_{f} \mid \psi - \pi_{N}^{\perp}(\psi) \rangle \right) \\
			&= \sup_{\| \psi \|_{H^{s}}=1} \left( 2 \Re \langle \pi_{N}^{\perp}(\psi) \mid H_{f} \mid \psi \rangle - \langle \pi_{N}^{\perp}(\psi) \mid H_{f} \mid \pi_{N}^{\perp}(\psi) \rangle \right) \\
			&\leq \sup_{\| \psi \|_{H^{s}}=1}  ( \| \pi_{N}^{\perp}(\psi) \|_{L^{2}}- \| \pi_{N}^{\perp}(\psi) \|_{L^{2}}^{2} ) \| H_{f} \|_{op} 
	\end{align*}
...
\begin{proof}
	We find
	\begin{align*}
		\| H_{f(t)} - H_{f,N}(t) \|_{H^{s},op} &= \sup_{\| \psi \|_{H^{s}} =1} \langle \psi \mid H_{f(t)} - H_{f,N}(t) \mid \psi \rangle
	\end{align*}
	In light of Proposition \ref{prop:isospectral} we find
	\begin{align*}
			\quad = \sup_{ \| \psi \|_{H^{s}} =1} \langle \psi \mid U(t) \cdot H_{f(0)} \cdot U(t)^{\dagger} - U_{N}(t) \cdot H_{f,N}(0) \cdot U_{N}(t)^{\dagger} \mid \psi \rangle.
	\end{align*}
	...
	\begin{align*}
			= \sup_{ \| \psi \|_{H^{s}} =1 } \langle U(t)^{\dagger} \psi \mid H_{f(0)} \mid U(t)^{\dagger} \psi \rangle
				- \langle U_{N}(t)^{\dagger} \psi \mid \pi_{N}^{\perp} \circ H_{f(0)} \circ \pi_{N}^{\perp} \mid U_{N}(t)^{\dagger} \psi \rangle.
	\end{align*}
	and finally
	\begin{align}
			&= \sup_{ \| \psi \|_{H^{s}} = 1} \langle U(t)^{\dagger} \psi \mid H_{f(0)} - H_{f,N}(0) \mid U(t)^{\dagger} \psi \rangle 
