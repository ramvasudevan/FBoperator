...
Before proceeding to describe any algorithms, let us take a moment to reflect on the value of pursuing qualitative accuracy.
We will show that PDEs of the form \eqref{eq:function pde} for continuous vector-fields, $X$, form the unique class of evolution PDEs which preserves the algebra of $C(M)$.
In other-words, that \eqref{eq:function pde} preserves $C(M)$ is its defining characteristic.
It would seem self-evident that the definition of \eqref{eq:function pde} be worthy of some respect during discretization.

As mentioned, the solution to \eqref{eq:function pde} with initial condition $f_{0} \in C(M)$ is given by $f(x,t) = f_{0}( (\Phi_{X}^{t})^{-1}(x)  )$.
Algebraically, the solution is obtained by a time-dependent linear transformation of $f_{0}$ given by $f_{0} \circ (\Phi_{X}^{t})^{-1}$.
This transformation, known as a ``Koopman operator'', preserves $C(M)$ as a Banach algebra in that it preserves the sup-norm as well as addition, multiplication, and complex conjugation.
One goal in this paper is to approximate $C(M)$ with a finite dimensional algebra, $A$, and then to approximate the Koopman operator with an algebra preserving transformation of $A$.
But is this a worthy goal?
Sure, \eqref{eq:function pde} preserves the algebraic structure of $C(M)$, but is that special?
Perhaps algebra-preservation is an insignificant property of \eqref{eq:function pde} in that it can be subsumed by a much larger conservation law, if only we knew what that larger law was?
Such a scenario would be analogous to declaring ``Holy cow!  We can encode all english words using the numbers $0$ through $25$!''.
This insight is true, but fails to realize the more fundamental truth ``We can encode using the numbers $0$ and $1$.''
Conversely, perhaps aiming for algebra-preservation is over-kill.  Perhaps algebra-preservation is a property of a more general class of linear evolution PDE's which are more difficult to solve?
This scenario would be analogous to the declaration ``Holy cow! We can assert the existence of a closed form solution to `$3x+1=0$' using Galois theory?''.
True, but the use of Galois theory is too general.

In fact, both of these concerns are needless.  Characterizing \eqref{eq:function pde} as an algebra-preserving PDE is niether overkill, nor is it insignificant.
...
Here we briefly illustrate why this is true by leveraging the theory of $C^{*}$-algebras.
...
\begin{definition}\label{def:half density}
	A half-density is a $C^{1}$ complex-valued function $\psi : \bigoplus^n TM \to \mathbb{C}$
...
\todo[inline]{Use word semi-discretization describe.}
