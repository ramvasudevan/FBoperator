...
We begin by proving an approximation bound between half densities in $V_{N} = \operatorname{span}( e_{1},\dots,e_{N})$ \ram{may just want to put in a preliminary notation section} and $H^{s}(M;g)$.
In all that follows, let $\pi_{N}: L^{2}(M) \to V_{N}$ denote the orthogonal projection.

First let us bound the approximation error induced by approximating the space of all half-densities with $V_{N}$.
...
\begin{proof}
	...
	The results are unchanged upon applying Assumption \ref{ass:basis} and converting everything to the Fourier basis.
	For any $\psi \in H^{s}(M;g)$ we can expand it in our basis as $\psi = \hat{\psi}_{k} e_{k}$ where $\hat{\psi}_{k} = \langle e_{k} \mid \psi \rangle$.
