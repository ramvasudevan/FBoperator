...
Unfortunately the spectral algorithm given above is not \emph{qualitatively accurate}, as shall be demonstrated.
...
In this article, we consider a unitary representation of the diffeomorphisms of $M$ known to representation theorists~\cite{VershilGelfandGraev1975,Ismagilov1975} and quantum probability theorists ~\cite{Meyer1998,Parthasarathy2012}.
To be more specific, we will consider the action of diffeomorphisms on the Hilbert space of half-densities~\cite{GuilleminSternberg1970,BatesWeinstein1997}.
...
Similarly, an alternative notion of half-densities is invoked in~\cite{Crane2013} to transform the a mean-curavature flow PDE into a better behaved one.
...
Finally, we prove our schemes are qualitatively accurate (\`a la Definition \ref{def:quality}) in section \ref{sec:qualitative}.
...
\subsection{Notation}
Throughout the paper $M$ will denote a smooth compact $n$-manifold without boundary. The space of continuous complex valued functions will be denoted $C(M)$
and we will topologize $C(M)$ by the sup-norm, $\| \cdot \|_{\infty}$ (see ~\cite{Taylor1974,Rudin1991,MTA,Conway1990}).
Given a Riemannian metric, $g$, the resulting Sobolev spaces on $M$ will be denoted $W^{k,p}(M ; g)$ (see ~\cite{Hebey1999}).
The tangent bundle to $M$ will be denoted by $TM$, and the $n$th iterated Whitney sum will be denoted by $\bigoplus^{n} TM$ (see ~\cite{Lee2006,MTA}).
A (complex) density 
will be viewed as a continuous map $\rho: \bigoplus^{n}TM \to \mathbb{C}$ which satisfies certain geometric properties which permit a notion of integration.
...
Given a separable Hilbert space $\mathcal{H}$ we will denote the Banach-algebra of bounded operators by $B( \mathcal{H})$
and topological group of unitary operators by $U( \mathcal{H})$.
...
The commutator bracket for operators $A,B$ on $\mathcal{H}$ will be denoted by $[A,B] := A \cdot B - B \cdot A$ (see~\cite{Conway1990}).
...
Before we describe our algorithms, let us take a moment to reflect on the virture of pursuing qualitative accuracy.
...
Because of this fact, many nice properties held by \eqref{eq:function pde} and \eqref{eq:density pde} (such as bounds) will also be held by a qualitatively accurate integration scheme.
Therefore, qualitatively accurate schemes leverage the defining aspects of the \eqref{eq:function pde} and \eqref{eq:density pde} in order to produce numerical approximations with the same qualitative characteristics.

In the remainder of this section we will illustrate how \eqref{eq:function pde} is the unique PDE which preserves $C(M)$ as an algebra.
In the interest of space, we will be providing references in place of proofs.
...
\begin{theorem}[~\cite{GelfandNaimark1943}] \label{thm:GN2}
...
In this paper, we will go the opposite direction.
We will start with an equation on the (concrete) space $C(M)$ for a compact manifold $M$
and we will transform it into an equation on a commutative sub-algebra of $B( \mathcal{H})$ for a suitably chosen Hilbert space, $\mathcal{H}$.
That we seemingly transform ``concrete'' objects into ``abstract'' objects is one possible reason that
the algorithms in this paper were not constructed earlier.
However, ``abstract'' does not necessarily imply difficult, with respect to numerics.
In fact, as this paper will show, it is easier to represent advection equations in this operator-theoretic form.
...
Said more plainly, \eqref{eq:function pde} and \eqref{eq:density pde} are just
the generators of all algebra-preserving automorphisms of $C(M)$.
...
Conservation of sums and products is the defining property of  \eqref{eq:function pde} and \eqref{eq:density pde}.
It would seem natural for this to be reflected in a discretization\footnote{Note: By aiming for qualitative accuracy without sacrificing spectral convergence, we reduce the coefficient of convergence.
Therefore this pursuit makes sense from the standpoint of ``old-school'' numerical accuracy as well.}.
...
At the core of any Galerking scheme, including spectral Galerkin, is the use of a Hilbert space upon which everything can be approximated via least squares projections.
...
The $L^{2}$-space we will use is slightly different, and this fact permeates the entire article.}.
