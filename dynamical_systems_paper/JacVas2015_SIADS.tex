...
To be more specific, we will consider the action of diffeomorphisms on the Hilbert space of half-densities~\cite{BatesWeinstein1997,GuilleminSternberg1970}.
...
Our auxiliary scheme will induces numerical schemes for \eqref{eq:function pde} and \eqref{eq:density pde} via Theorem \ref{thm:quantize}.
Third, we derive a spectral convergence rate for our auxiliary scheme in Section \ref{sec:analysis}.
The spectral convergence rate for our auxiliary scheme will induce spectral convergence rates for numerical schemes for \eqref{eq:function pde} and \eqref{eq:density pde}.
...
\begin{theorem}[\cite{GelfandNaimark1943} \ram{Probably want to cite the theorem in the citation}] \label{thm:GN2}
	Any commutative $C^{*}$-algebra, $A$, is isomorphic to $C(X_{A} )$.
\end{theorem}
...
We start with an equation on the (concrete) space $C(M)$ for a compact manifold $M$ \ram{may want to cite the equation that is relevant in the text} and we transform it into an equation on a commutative sub-algebra of $B(\mathcal{H})$ for a suitably chosen Hilbert space, $\mathcal{H}$ \ram{may want to cite the equation that is relevant in the text}.
...
\end{align}
for any $x \in M$ and any $v_{1}, \dots , v_{n} \in T_{x}M$ \ram{can you describe in a single sentence how you generated this equation?}.
...
The metric also induces an elliptic operator, known as the Laplace-Beltrami operator $\Delta: C^{\infty}(M) \to C^{\infty}(M)$, which is negative-semidefinite (i.e. $\int f \, \Delta f \, \mu_{g} \leq 0$ for all $f \in C^{2}(M)$) \ram{Is it worth just defining this equation in this paper?}.
...
Such a definition is isomorphic, in the category of topological vector-spaces, to the one provided in ~\cite{Hebey1999} \ram{don't you need to prove this fact?}.
...
\end{align}
for any $f \in C(M)$.
More generally, for any $A$ in the dual-space $B( L^{2}(M) )^{*}$ \ram{does the * need to be in the previous equation}, there is a $\rho_{A} \in C(M)'$ such that $\langle f , \rho_{A} \rangle = \langle H_{f} , A \rangle$.
...
More explicitly, let $\OdeSolve ( F , x_{init} , t_{final} )$ denote the numerically computed solution $x(t_{final})$ to the ODE ``$\dot{x} = F(x)$'' at time $t_{final}$, with initial condition $x_{init}$ \ram{OdeSolve may need arguments corresponding to the time interval of integration}.
...
\begin{algorithm}
	...
	$t_{0} = 0$\;
	\For{ k = 1,\dots,m}{
		$z(t_{k}) = \operatorname{OdeSolve}(  F , z(t_{k-1}) , t_{k})$ \ram{OdeSolve may need arguments corresponding to the time interval of integration}\;
	}
...
\ram{Can you summarize the importance of the following result in a single sentence?}
...
\begin{theorem} \label{thm:sparsity}
	...
	If differentiation and multiplication of the functions $f_{k}$ is sparse (in terms of series expansions) \ram{you may want to make more clear what this means and how it relates to the bound...} and if the vector-field $X$ is such that $X^{i} = \sum_{k} c_{k}^{i} f_{k}$ with fewer than $K>0$ of $c^{i}_{k}$'s being non-zero for each $i=1,\dots,n$, then the matrix $X_{N}$ in Algorithm \ref{alg:half density} is banded diagonal and the sparsity of $X_{N}$ is $\mathcal{O}( K / N )$.
\end{theorem}
...
This algorithm is presented for theoretical interest at the moment, as it does not scale well with the problem size in terms of memory usage \ram{the second part of this sentence may not be worth including as it detracts from the punchline, there maybe a safer way to explain this}.
...
\begin{algorithm}[H]
	...
	$t_{0} = 0$\;
	\For{ $k = 1 ,\dots, m $}{
		$F_{N}(t_{k}) = \OdeSolve( B , F_{N}(t_{k-1}) , t_{k})$ \ram{OdeSolve may need arguments corresponding to the time interval of integration}\;
	}
...
\ram{a single sentence describing the relevance of the following result would be useful.}
\begin{proposition} \label{prop:isospectral}
\ram{it is worth stating the hypothesis to this Proposition analogously to Proposition \ref{prop:unitary}. Its also strange that things here are in terms of $t$ but earlier things are in terms of $t_k$. This is particularly relevant given comments that I'll make about the next section.}
$H_{f,N}(t) = U_{N}(t) \cdot H_{f,N}(0) \cdot U_{N}(t)^{\dagger}$ for all time $t$ for the same unitary transformation $U_{N}(t)$ of Proposition \ref{prop:unitary}.
...
\ram{Three comments: First, throughout this section you describe solutions generated by the Algorithms as being true for all times so that you can define the error analysis easily, but in practice the output of each algorithm is just defined for $t_k$'s. Generally speaking linear interpolation is used between sample points, but this isn't mentioned here. Its also weird because the times are allowed to be selected arbitrarily. Second, in Prop 6.1 you use subscripts on constants to denote notational dependence, but in subsequent propositions within this section you don't use the same notational convention. I like the notation and it may be worth using consistently. Third, it makes sense for each convergence statement about each algorithm to be stated as similarly as possible. At the moment each one seems very very distinct.}.

In this sections we derive convergence rates for all the algorithms mentioned thus far.
...
\begin{proposition} \label{prop:function approximation}
	Let $f \in C^{k}(M)$ and let $\hat{f}_{N} = \pi_{N} \circ \hat{f} \circ \pi_{N}$ be the output of Algorithm \ref{alg:function} \ram{why does it have to be an output of Algorithm \ref{alg:function}? I mean this is just a projection result right?}.  Then
	\begin{align}
...
\begin{theorem} \label{thm:norms}
	...
	Then $\| \psi_{N} (t)\|_{L^{2}}, \|\rho_{N}(t)\|_{L^{1}},$ and $\| H_{f,N}(t) \|_{op}$ are constant with respect to $t$ for arbitrary $N \in \mathbb{N}$.
	Moreover,
	\begin{align*}
		\lim_{N \to \infty} \| \psi_{N} (t) \|_{L^{2}} = \| \psi(\cdot; t) \|_{L^{2}}, \\
		\lim_{N \to \infty} \| \rho_{N}(t) \|_{nuc} = \| \rho(\cdot; t) \|_{L^{1}}, \\
...
\ram{For the benchmark examples it may be useful to always use the same color for the spectral method (green) and our method (red) on all of the plots (the flow, the convergence, and the norm preservation results). Maybe the ground truth can be plotted in blue and the discrepancy can be plotted in blue too.}
...
\begin{figure}[h]
	...
	\caption{A plot of the sup-norm vs time of a standard spectral discretization (blue) and the result of Algorithm \ref{alg:function} (green) on the example described in Section \ref{sec:benchmark}.}
	\label{fig:norms}
\end{figure}
