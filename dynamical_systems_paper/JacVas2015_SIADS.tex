...
Such PDEs arises in a variety of contexts, ranging from  mechanics \cite{Batchelor1999,Truesdell1991} to control theory \cite{HenrionKorda2014}, and can be seen as zero-noise limits of the Forward/Backward Kolmogorov equations \cite{Oksendal2003}.
...
\begin{enumerate}
	...
	\item We will prove our schemes are qualitatively accurate (i.e. preserves scalar addition/multiplication, total mass, $L^{1}-norms$, etc.) in section \ref{sec:qualitative}
	\item We will demonstrate these findings in numerical experiments in section \ref{sec:numerics}.
	Furthermore, we will observe our algorithm for \eqref{eq:density pde} to be superior to a standard spectral discretization.
\end{enumerate}
...
\begin{theorem} \label{thm:quantize}
	...
	Moreover, if $\psi \in L^{2}(M)$ is such that $\psi^{2} = \rho$, then $\psi$ satisfies \eqref{eq:half density pde}
...
\section{Semi-discretizations and algorithms on compact manifolds} \label{sec:discretization}
