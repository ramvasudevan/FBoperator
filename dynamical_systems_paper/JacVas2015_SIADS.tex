...
\begin{itemize}
	...
	\item The space of densities on $M$ will be denoted by $\Dens(M)$ 
	and the integral of a density $\rho \in \Dens(M)$ will be denoted by $\int \rho$.
	...
	In fact $L^{1}(M)$ is just the (pre)-dual space to $C(M)$.  Given a function $f \in C(M)$ we denote the multiplication of the density $\rho \in L^{1}(M)$ by $f \rho$
...
\noindent In fact, $U(t)$ is merely the operator which deforms $\psi$ by the flow of $X$ according to formula \eqref{eq:transformation law}.
...
and the subspace
\begin{align}
	V^{s}(M;g) = \{ \psi \in \sqrt{\Dens(M)} \mid \psi = f \, \sqrt{\mu}_{g} , \| f \|_{s,2} < \infty \}.
\end{align}
Then $H^s(M ;g)$ is the completion of $V^{s}(M;g)$ with respect to the $\| \cdot \|_{s,2}$ norm.  Such a definition is isomorphic, in the category of topological vector-spaces, to the one provided in \cite{Hebey1999}.
...
\begin{ass} \label{ass:basis}
	Our basis $\{ e_{k} \}$ is such that there exists a metric $g$ for which the unitary transformation which sends the basis $\{ e_{k} \}$ to the Fourier basis is bounded on $H^{s}(M;g)$ for some $s > 1$.
\end{ass}
...
We observe that algorithm \ref{alg:function} involves the integration of an isopectral flow \cite{Calvo1997},
and so the solutions are of the form $U_{N}(t) \cdot f_{N}(0) U_{N}(t)$ for all time $t$ for some unitary transformation $U_{N}(t)$.
In fact, it is the same unitary transformation as the one in Proposition \ref{prop:unitary}.
...
\begin{proof}
	...
	As $U_{N}(t)^{\dagger} \psi$ is the backwards time numerical solution to \eqref{eq:half density pde} and $U_{N}(t)^{\dagger}\psi$ is the exact backward time solution to \eqref{eq:half density pde},
...
\begin{theorem} \label{thm:norms}
	Let $\psi_{N}(t)$, $\rho_{N}(t)$, and $H_{f,N}(t)$ denote the outputs of Algorithms \ref{alg:half density}, \ref{alg:density}, and \ref{alg:function}
	with respect to initial conditions $\psi(0) \in H^{s}(M;g), \rho(0) \in W^{s,1}$ and $f(0) \in C^{s}(M)$.
	Then $\| \psi_{N} (t)\|_{L^{2}}, \|\rho_{N}(t)\|_{L^{1}},$ and $\|\hat{f}_{N}(t) \|_{op}$ are constant with respect to $t$ and
	\begin{align}
