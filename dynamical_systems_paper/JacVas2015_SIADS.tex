...
We observe our algorithm for \eqref{eq:density pde} to be superior to a standard spectral Galerkin discretization.
...
A (complex) density 
will be viewed as a continuous map $\rho: \bigoplus^{n}TM \to \mathbb{C}$ which satisfies certain geometric properties which allow for integration.
...
We associate the norm $\| \rho \|_{1} =  \int | \rho |$ to densities.
By completion this builds a Banach space, which we call $L^{1}(M)$ (slightly different from the traditional construction).
The space, $L^(M)$ is homeomorphic to the space of distributions upon a (non-canonical) choice of Lebesgue measure.
...
Given a separable Hilbert space $\mathcal{H}$ we will denote the space of bounded operators by $B( \mathcal{H})$
...
The commutator bracket for operators $A,B$ on $\mathcal{H}$ will be denoted by $[A,B] := A \cdot B - B \cdot A$ (see Chapter 9 of \cite{Conway1990}).s
...
Therefore, qualitatively accurate schemes leverage the best aspects of the PDEs \eqref{eq:function pde} and \eqref{eq:density pde} to produce numerical approximations with similar behavior to \eqref{eq:function pde} and \eqref{eq:density pde}.
...
\begin{example} \label{ex:nc algebra} 
	For a Hilbert space, $\mathcal{H}$, the space of bounded operators, $B(\mathcal{H})$, is a (non-commutative) $C^{*}$-algebra	under operator multiplication and addition, with the involution $L \mapsto L^{\dagger}$, and the operator-norm.
\end{example}
...
\begin{theorem}[\cite{GelfandNaimark1943}] \label{thm:GN2}
	Any commutative $C^{*}$-algebra, $A$, is isomorphic to $C(X_{A} ; \mathbb{C})$.
\end{theorem}
...
(as described by the Definition \ref{def:algebra}) into Examples \ref{eq:function algebra} and \ref{eq:nc algebra}.
In this paper, we will go the opposite direction.
We will start with an equation on the (concrete) space $C(M;\mathbb{C})$ for a compact manifold $M$
and we will transform it into an equation on $B( \mathcal{H})$ for a suitably chosen Hilbert space, $\mathcal{H}$.
That we will turn ``concrete'' objects into ``abstract'' objects is one possible reason, that
the algorithms in this paper were not considered earlier.
...
\begin{corollary}[follows from Corollary 1.7 of \cite{Bondia2001}]
	...
	Moreover, a linear evolution equation on $C^{0}(M)$ given by $\partial_{t} f + A[f] = 0$ preserves 
	the algebra of $C(M;\mathbb{C})$ if and only if $F[f] = X^{i} \pder{f}{x^{i}}$ for some vector-field $X$.
...
It would seem natural for this to be reflected in a discretization.

To end this section, let us reiterate our goal.  We desire a scheme with a spectral rate of convergence and qualitative accuracy.  Improving the numerical accuracy of a spectral scheme is quite difficult, because spectral convergence rates are about the best anyone should expect to achieve.  By aiming for qualitative accuracy without sacrificing spectral convergence, we will find that we reduce the coefficient of convergence.
So this pursuit makes sense from the standpoint of ``old-school'' numerical accuracy as well.
