...
In order to ensure convergence we will find that this basis must have the property that $\| e_{k} \|_{H^{s}}$ is asymptotically proportional to $k^{2s/d}$ where $d = \dim(M)$ for some $s \geq 1$.
...
Before solving \eqref{eq:function pde} and \eqref{eq:density pde}, let us solve \eqref{eq:half density pde}.
...
\begin{algorithm}
	...
	initialize the function $F: \mathbb{C}^{n} \to \mathbb{C}^{n}$
	given by $F(z) = X_{n} \cdot z$.\;
	\For{ $i = 1 , \dots , n$}{
...
By theorem \ref{thm:quantize} we can merely square the result to obtained a numerical solution to \eqref{eq:density pde}.

This yields the following numerical solution for densities which admit square roots.
For example for any continuous $\rho \in L^{1}(M)$ we may let $\rho^{+}$ denote the positive part and $\rho^{-}$ denote the negative part so that $\rho = \rho^{+} - \rho^{-}$.
