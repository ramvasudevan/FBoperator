...
\begin{enumerate}
	...
	\item C*algebra and noncommutative geometry (with caveat that we don't know any).
\end{enumerate}
...
\begin{thm} \label{thm:convergence 0}
...
\begin{proof}
	...
	\begin{align*}
		\| \psi - \pi_{n}(\psi) \|_{H^{s}} \leq \tilde{C} \sum_{k>n} \frac{1}{k^{1+2(\bar{s}-s)/d}} = \zeta \left( 1+2\frac{\bar{s}-s}{d} \right) - \sum_{k=1}^{n} \frac{1}{k^{1+2(\bar{s}-s)/d}}.
	\end{align*}
	Where $\zeta$ denotes the Riemann zeta function.
	The finite-sum approximates the Riemann zeta function with an error dominated by something proportional to $\frac{d }{ \bar{s}-s} n^{-2(\bar{s}-s)/d}$.\todo{Find a good citation}
\end{proof}


\begin{thm} \label{thm:convergence 1}
	Let $\psi_{0} \in H^{s}(M)$ for $s > 1$
	Let $\psi(t)$ be the exact solution to \eqref{eq:half density pde} with initial condition $\psi_{0}$ and let
	and $\psi_{n}(t)$ be the $n$th numerical solutions on the interval $[0,T]$ for $T < \infty$ with initial condition $\psi_{n}(0) = \pi_{n}(\psi_{0})$.
	Then the error $E_{n}(t) := \| \psi(t) - \tilde{\psi}(t) \|_{H^{s}}$ satisfied the bound
	\begin{align*}
		E_{n}(t) \leq ( K_{T} n^{-2(s-1)} t+  E_{n}(0) ) e^{C_{T}}
	\end{align*}
	for $t \in [0,T]$ and constants $K_{T}$ and $C_{T}$.
	Moreover, if $\psi_{0} \in H^{\bar{s}}$ for $\bar{s} > s$ then $E_{n}(0)$ behaves as $n^{-2(\bar{s}-s)/d}$.
\end{thm}
...
\begin{proof}
	...
	\begin{align*}
		\frac{dE}{dt} \leq C_{1,T} \, \| (1- \pi_{n})|_{H^{s-1}} \| + C_{2,T} E(t)
	\end{align*}
	...
	Therefore by a perturbed version of Gronwall's inequality (see Lemma \ref{lem:Gronwall} on page \pageref{lem:Gronwall}) with $\epsilon \sim n^{-2(s-1) / d}$
	we find
	\begin{align*}
		E(t) \leq ( K_{T} n^{-2(s-1) / d} t+  E(0) ) e^{C_{2,T}}.
	\end{align*}
	That $E_{n}(0)$ behaves as $n^{-2(\bar{s}-s)/d}$ is merely a re-statement of Theorem \ref{thm:convergence 0}
\end{proof}
