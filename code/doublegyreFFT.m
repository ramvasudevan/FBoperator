...
for j = 1:length( t )
    for k = 1:length( t )
        f1( j, k ) = pi^2 * cos( pi * t( j ) ) * cos( pi * t( k ) );
        f2( j, k ) = pi * sin( pi * t( j ) ) * cos( pi * t( k ) );
        f3( j, k ) = pi^2 * cos( pi * t( j ) ) * cos( pi * t( k ) );
        f4( j, k ) = pi * cos( pi * t( j ) ) * sin( pi * t( k ) );
    end
end
...
%     X2( h_idx, k ) = -f3vec( f_idx ) - 1i * c_idx( 2 ) * f4vec( f_idx );
    X( h_idx, k ) = f1vec( f_idx ) - f3vec( f_idx ) + 1i * ( c_idx( 1 ) * f2vec( f_idx ) - c_idx( 2 ) * f4vec( f_idx ) );
end
toc
