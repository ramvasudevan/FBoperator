...
%% plot vector field
[ Du, Dv, ~, ~ ] = ConstR2( u, v );
[ ~, Y ] = ode23( @ConstR2_plot, [0 1], [ 0 -0.1 ] );
% figure; hold on;
% quiver( u, v, Du, Dv );
% plot( Y( :, 1 ), Y( :, 2 ), 'g' );
...
syms x1 x2;
H = matlabFunction( ( heaviside( x1 + 2.5 ) - heaviside( x1 + 1.5 ) ) * ( heaviside( x2 + 1/2 ) - heaviside( x2 - 1/2 ) ) );
% H = matlabFunction( ( heaviside( x1 + 0.5 ) - heaviside( x1 - 0.5 ) ) * ( heaviside( x2 + 5/2 ) - heaviside( x2 + 3/2 ) ) );
F = Weights .* H( tx, ty );
evo0 = Trig * ( Weights .* H( tx, ty ) );

% length of time to plot
T = linspace( 0, 0.25, 10 );
