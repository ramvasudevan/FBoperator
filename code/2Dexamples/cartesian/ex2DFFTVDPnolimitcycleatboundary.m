...
[ tx, ty ] = meshgrid( t );
% h1 = matlabFunction( scaling * x2 * ( pi^2 - x2^2 )^4 * ( pi^2 - x1^2 )^4 );
% h2 = matlabFunction( -(scaling*x1 + scaling*vdpscale*x2.*((scaling*x1).^2 - 1)) * ( pi^2 - x2^2 )^4 * ( pi^2 - x1^2 )^4 );
% d1 = matlabFunction( diff( scaling * x2 * ( pi^2 - x2^2 )^4 * ( pi^2 - x1^2 )^4, x1 ) );
% d2 = matlabFunction( diff( -(scaling*x1 + scaling*vdpscale*x2.*((scaling*x1).^2 - 1)) * ( pi^2 - x2^2 )^4 * ( pi^2 - x1^2 )^4, x2 ) );
% f1 = -h1( tx, ty );
% f2 = -d1( tx, ty );
% f3 = -h2( tx, ty );
% f4 = -d2( tx, ty );
% [ f1, f3, f2, f4 ] = vdpS1( tx, ty );
...
f1vec = interp2( freqsx, freqsy, F1, gridx, gridy, 'nearest' );
f2vec = interp2( freqsx, freqsy, F2, gridx, gridy, 'nearest' );
f3vec = interp2( freqsx, freqsy, F3, gridx, gridy, 'nearest' );
f4vec = interp2( freqsx, freqsy, F4, gridx, gridy, 'nearest' );
toc
