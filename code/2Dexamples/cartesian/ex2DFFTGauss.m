...
[ tx, ty ] = meshgrid( t );
h1 = @(x1,x2) -x1 .* exp(-1./(1-(x1/pi).^100)) .* exp(-1./(1-(x2/pi).^100)) * (exp(1))^2;
h2 = @(x1,x2) -x2 .* exp(-1./(1-(x1/pi).^100)) .* exp(-1./(1-(x2/pi).^100)) * (exp(1))^2;
d1 = @(x1,x2) -(exp(-1./(1-x1.^100/pi^100)).*(x1.^200-102*pi^100*x1.^100+pi^200))./(pi^100-x1.^100).^2.* exp(-1./(1-(x2/pi).^100)) * (exp(1))^2;
d2 = @(x1,x2) -(exp(-1./(1-x2.^100/pi^100)).*(x2.^200-102*pi^100*x2.^100+pi^200))./(pi^100-x2.^100).^2.* exp(-1./(1-(x1/pi).^100)) * (exp(1))^2;
% h1 = @(x1,x2) x1 .* ( heaviside( x1 - pi + 1e-2 ) - heaviside( x1 + pi - 1e-2 ) ) .* ( heaviside( x2 - pi + 1e-2 ) - heaviside( x2 + pi - 1e-2 ) );
% h2 = @(x1,x2) x2 .* ( heaviside( x1 - pi + 1e-2 ) - heaviside( x1 + pi - 1e-2 ) ) .* ( heaviside( x2 - pi + 1e-2 ) - heaviside( x2 + pi - 1e-2 ) );
% d1 = @(x1,x2) (x1 .* (dirac( x1 - pi + 1e-2 )-dirac( x1 + pi - 1e-2 )) + heaviside( x1 - pi + 1e-2 ) - heaviside( x1 + pi - 1e-2 ) ) .* ( heaviside( x2 - pi + 1e-2 ) - heaviside( x2 + pi - 1e-2 ) );
% d2 = @(x1,x2) ( heaviside( x1 - pi + 1e-2 ) - heaviside( x1 + pi - 1e-2 ) ) .* (x2 .* (dirac( x2 - pi + 1e-2 )-dirac( x2 + pi - 1e-2 )) + heaviside( x2 - pi + 1e-2 ) - heaviside( x2 + pi - 1e-2 ) );
% h1 = @(x1,x2) pi * sin( x1 ) .* cos( x2 );
% h2 = @(x1,x2) -pi * cos( x1 ) .* sin( x2 );
% d1 = @(x1,x2) pi * cos( x1 ) .* cos( x2 );
% d2 = @(x1,x2) -pi * cos( x1 ).* cos( x2 );
f1 = h1( tx, ty );
f2 = d1( tx, ty );
f3 = h2( tx, ty );
f4 = d2( tx, ty );
...
if ( compute_nulls )
    [ helperL, helperR ] = spspaces( Xnum, 3, 1e-3 );
    VR = helperR{1};
    zeroeigR = helperR{3};
    VL = helperL{1};
    zeroeigL = helperL{3};
else
    [ V, D ] = eig( Xnum, eye( size( Xnum ) ) );
