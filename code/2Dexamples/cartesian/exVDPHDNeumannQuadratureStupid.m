...
for k = 1:size( multi_idx, 1 )
    ...
    if( mod( cx, 2 ) && mod( cy, 2 ) )
        ...
        DfTrig( k, :, : ) = f1 .* cx/2 .* cos( cx * tx/2 ) .* sin( cy * ty/2 ) + f2 .* cy/2 .* sin( cx * tx/2 ) .* cos( cy * ty/2 ) + ...
            1/2 .* ( f3 + f4 ) .* sin( cx * tx/2 ) .* sin( cy * ty/2 );
    elseif( mod( cx, 2 ) )
        Trig( k, :, : ) = sin( cx * tx/2 ) .* cos( cy * ty/2 );
        DfTrig( k, :, : ) = f1 .* cx/2 .* cos( cx * tx/2 ) .* cos( cy * ty/2 ) + f2 .* -cy/2 .* sin( cx * tx/2 ) .* sin( cy * ty/2 ) + ...
            1/2 .* ( f3 + f4 ) .* sin( cx * tx/2 ) .* cos( cy * ty/2 );
    elseif( mod( cy, 2 ) )
        Trig( k, :, : ) = cos( cx * tx/2 ) .* sin( cy * ty/2 );
        DfTrig( k, :, : ) = f1 .* -cx/2 .* sin( cx * tx/2 ) .* sin( cy * ty/2 ) + f2 .* cy/2 .* cos( cx * tx/2 ) .* cos( cy * ty/2 ) + ...
            1/2 .* ( f3 + f4 ) .* cos( cx * tx/2 ) .* sin( cy * ty/2 );
    else
        Trig( k, :, : ) = cos( cx * tx/2 ) .* cos( cy * ty/2 );
        DfTrig( k, :, : ) = f1 .* -cx/2 .* sin( cx * tx/2 ) .* cos( cy * ty/2 ) + f2 .* -cy/2 .* cos( cx * tx/2 ) .* sin( cy * ty/2 ) + ...
...
for k = 1:size( multi_idx, 1 )
    ...
    for l = 1:size( multi_idx, 1 )
        henry( l ) = sum( sum( Weights .* wales .* squeeze( Trig( l, :, : ) ) ) );
    end
    X( k, : ) = henry;
end
toc
