...
h1 = matlabFunction(  ( 1/2 + x1 ) * ( pi^2 - x1^2 ) );
d1 = matlabFunction( diff( ( 1/2 + x1 ) * ( pi^2 - x1^2 ) , x1 ) );
...
for k = 1:size( multi_idx, 1 )
    ...
    if( mod( c_idx, 2 ) )
        ...
        helper( :, 1 ) = c_idx/4 * f1vec( h_idx ) - 1i/2 * f2vec( h_idx );
        
        s_idx = -c_idx/2 + sampled_freqs';
        h_idx = intersect( find( s_idx >= -NBases ), find( s_idx <= NBases ) );
        helper( :, 2 ) = c_idx/4 * f1vec( h_idx ) + 1i/2 * f2vec( h_idx );
    else
        % cos( kx/2 ) k is even case
        s_idx = c_idx/2 + sampled_freqs';
        h_idx = intersect( find( s_idx >= -NBases ), find( s_idx <= NBases ) );
        helper( :, 1 ) = 1i * c_idx/4 * f1vec( h_idx ) + 1/2 * f2vec( h_idx );
        
        s_idx = -c_idx/2 + sampled_freqs';
        h_idx = intersect( find( s_idx >= -NBases ), find( s_idx <= NBases ) );
        helper( :, 2 ) = -1i * c_idx/4 * f1vec( h_idx ) + 1/2 * f2vec( h_idx );
    end
    ...
    for n = 1:NSamples
        for k = 1:length( mod_multi_idx )
            if ( mod( mod_multi_idx( k ), 2 ) )
                ...
                %                 out( m, n ) = out( m, n ) + sum( V( k, : ) ) * sin( mod_multi_idx( k ) * x( n )/2 );
            else
                out( m, n ) = out( m, n ) + V( k, zeroeig( m ) ) * cos( mod_multi_idx( k ) * x( n )/2 );
                %                 out( m, n ) = out( m, n ) + sum( V( k, : ) ) * cos( mod_multi_idx( k ) * x( n )/2 );
            end
        end
