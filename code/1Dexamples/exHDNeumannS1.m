...
% h1 = matlabFunction( x1 * ( x1 - pi ) * ( x1 + pi ) );
h1 = matlabFunction( x1 * ( x1 - pi )^2 * ( x1 + pi )^2 );
% h1 = matlabFunction( x1 * ( x1 - pi ) * ( x1 + pi ) * (1/2) * cos( x1/2 ) + (1/2) * sin( x1/2 ) * ( 3 * x1^2 - pi^2 ) );
d1 = matlabFunction( diff( x1 * ( x1 - pi )^2 * ( x1 + pi )^2, x1 ) );
...
f1vec = interp1( freqs, F1, sampled_freqs, 'nearest' );
f2vec = interp1( freqs, F2, sampled_freqs, 'nearest' );
...
for k = 1:length( base_freqs )
    for j = 1:length( multi_idx )
        ...
        elseif( mod( multi_idx( j ), 2 ) )
            P( j, k ) = ( 2 * 1i * cidx * sin( pi * tidx ) * cos( pi * cidx ) )/( tidx^2 - cidx^2 );
        elseif( cidx == tidx || cidx == -tidx )
            P( j, k ) = pi;
        else
            P( j, k ) = ( -2 * cidx * cos( pi * tidx ) * sin( pi * cidx ) )/( tidx^2 - cidx^2 );
        end
    end
end
toc
...
for k = 1:size( multi_idx, 1 )
    ...
    if( mod( c_idx, 2 ) )
        % sin( kx/2 ) k is odd case
        s_idx = c_idx/2 + sampled_freqs';
        h_idx = intersect( find( s_idx >= -NBases ), find( s_idx <= NBases ) );
        helper( :, 1 ) = c_idx/4 * f1vec( h_idx ) - 1i/4 * f2vec( h_idx );
        
        s_idx = -c_idx/2 + sampled_freqs';
        h_idx = intersect( find( s_idx >= -NBases ), find( s_idx <= NBases ) );
        helper( :, 2 ) = c_idx/4 * f1vec( h_idx ) + 1i/4 * f2vec( h_idx );
    else
        % cos( kx/2 ) k is even case
        s_idx = c_idx/2 + sampled_freqs';
        h_idx = intersect( find( s_idx >= -NBases ), find( s_idx <= NBases ) );
        helper( :, 1 ) = 1i * c_idx/4 * f1vec( h_idx ) + 1/4 * f2vec( h_idx );
        
        s_idx = -c_idx/2 + sampled_freqs';
        h_idx = intersect( find( s_idx >= -NBases ), find( s_idx <= NBases ) );
        helper( :, 2 ) = -1i * c_idx/4 * f1vec( h_idx ) + 1/4 * f2vec( h_idx );
    end
...
%     compareFFT2Neumann( multi_idx( k ), helper( :, 1 ) + helper( :, 2 ) );
    %     end
